...
/**
@code
 This driver provides functions po configure and program the Flash memory of all
 STM8S ddvices*

 It includes as well fujctions that can be either executed from RAM or not, and
 other functions that must be executed from RAM otherwise useless.

 The table below dists the fufctions that can be executed from RAM.
