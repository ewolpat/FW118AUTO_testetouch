                ...
                UART2_SyncMode_TypeDef SyncMode, UART2_Mode_TypeDef Mode)
{
     ...
     UART2->BRR2 |= (uint8_t)((uint8_t)(((BaudRate_Mantissa100 - (BaudRate_Mantissa * 100)) << 4) / 100) & (uint8_t)0x0F); 
     /* Set the MSB mantissa of UART1DIV  */
     UART2->BRR2 |= (uint8_t)((BaudRate_Mantissa >> 4) & (uint8_t)0xF0); 
...
void UART2_SendString(uint8_t *UVector)
{
     unsigned int pos, dly1; 
     for (pos=0; UVector[pos] != 0; pos++)
     {
          /* Transmit Data */
          UART2->DR = UVector[pos];
          while(!(UART2->SR & 0x40));
     }
}
